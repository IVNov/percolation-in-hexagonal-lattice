/*Программа перебирает num случайных раскрасок многоугольника для данного n (n>=3), 
и находит количество раскрасок, при котором из центра до границы 
	...
	nnn:{
			...
			prod:if ((polygon[h+2][v] ==1) || (polygon[h+2][v] ==k)) //если можно идти вниз, идём
				...
				else
				{
					...
					r (ставим границу) и изменяем значение g (пытаемся пойти в другую сторону)
					Если мы можем пойти в какую-нибудь клетку, то меняем значение r так, чтобы
